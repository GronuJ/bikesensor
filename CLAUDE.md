...
`bikesensor` is a hardware + software pipeline for geo-tagged bike vibration / road-quality mapping. An ESP32-C3 SuperMini logs 100 Hz accelerometer data and 1 Hz GPS to a MicroSD card during a ride; on arriving home it joins Wi-Fi and POSTs the CSVs to a FastAPI server on a Raspberry Pi (`bikesensor-server.local`), which runs the DSP pipeline and registers rides in SQLite. A Streamlit dashboard renders heatmaps and curb events.
...
`platformio.ini` sets `src_dir = bikesensor` so the `.ino` compiles in place. `firmware/diagnostic/diagnostic.ino` is a separate standalone bring-up sketch for checking SD/I2C/GPS wiring — it is *not* in the PlatformIO build; swap `src_dir` to compile it.
...
`process_unified_offline` falls back to a hardcoded Kiel coordinate (54.3486, 10.1176) when a log has fewer than 2 GPS fixes, producing a degenerate zero-length track. Rides that look like they happened at a single point are usually this, not a bug in the dashboard.
...
- `firmware/bikesensor/bikesensor.ino 2.cpp` is an untracked PlatformIO preprocessor artifact, not source.
