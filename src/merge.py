...
def process_unified_offline(offline_csv_path: str | Path,
    ...
    if len(gps_fixes) < 2:
         # Previously this substituted hardcoded Kiel CAU coordinates, which produced a
         # real-looking ride at a location the bike never visited. Once this data is
         # published that is not a cosmetic problem, so a ride without a usable track
         # is now rejected and kept as raw bytes for inspection instead.
         raise ValueError(
              f"Unified log has {len(gps_fixes)} GPS fix(es); at least 2 are required to "
              "build a track. Refusing to fabricate coordinates."
         )
